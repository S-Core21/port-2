...
menuOpen.addEventListener("click", () => {
  if(querry.matches){
  navBar.style.cssText = "transform: translateX(0px); transition: all 1s linear .2s"
...
menuClose.addEventListener("click", () => {
  navBar.style.cssText =
    "transform: translateX(300px); transition: all .1s linear 0s";
})
...
menuclose2[i].addEventListener('click', ()=>{
  if(querry.matches){
  navBar.style.cssText =
  "transform: translateX(300px); transition: all 2s linear 0s";
}
})
}
